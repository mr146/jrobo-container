...
public class Container implements IContainer {
    ...
    public <T> T get(Class<T> requiredAbstraction, HashSet<Class<?>> usedClasses) throws JRoboContainerException {
        try {
            ...
            synchronized (storage.getSynchronizeObject(resolvedClass)) {
                if (storage.getInstance(resolvedClass) == null) {
                    storage.putInstance(resolvedClass, getInstance(resolvedClass, usedClasses));
                }
            }
    ...
    public <T> T get(Class<T> requiredAbstraction) throws JRoboContainerException {
        return get(requiredAbstraction, new HashSet<Class<?>>());
    }

    @Override
    public <T> T create(Class<T> requiredAbstraction) throws JRoboContainerException {
        return create(requiredAbstraction, new HashSet<Class<?>>());
    }
    ...
    @SuppressWarnings("unchecked")
    public <T> T create(Class<T> requiredAbstraction, HashSet<Class<?>> usedClasses) throws JRoboContainerException {
        try {
    ...
    private <T> T getInstance(Class<T> resolvedClass, HashSet<Class<?>> usedClasses) throws InvocationTargetException, IllegalAccessException, InstantiationException, JRoboContainerException {
        ...
        for (int i = 0; i < parametersCount; i++) {
            parameters[i] = get(parametersTypes[i], usedClasses);
        }
